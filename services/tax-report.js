...
const TaxReport = (function () {
  ...
  function toAcbTxn(t) {
    if (t.type === 'roc') {
      return { type: 'roc', date: t.date, amount: num(t.amount), fxRate: num(t.fxRate) > 0 ? num(t.fxRate) : 1 };
    }
    return {
  ...
  function buildDispositions(positions, opts) {
    ...
    Object.keys(bySym).sort().forEach(sym => {
      ...
      const approx = group.some(p => (p.txns || []).some(t => t.source === 'broker-import' || t.fxEstimated));
      const merged = [];
      ...
      (result.ledger || []).forEach(r => {
        if (r.type === 'sell') {
          ...
          rows.push({
            ...
            proceedsCad: round2(num(r.capitalGain)), acbCad: 0,
